...
var Arrow = function(pos,vel) {
  ...
  if(vel.x==0 && vel.y==0 && vel.z==0) {
    height = ((1)**2 + (1)**2 + (1)**2)**(1/2) * 15 *2 ;
  } else if(vel.x==0 && vel.y==0) {
    height = ((1)**2 + (1)**2 + (vel.z)**2)**(1/2) * 15 *2 ;
  } else {
    height = ((vel.x)**2 + (vel.y)**2 + (vel.z)**2)**(1/2) * 15 *2 ;
  }
...
function color2hex(htmlColor) {
  if(!htmlColor.length) return 0xFFFFFF
  ...
  if(htmlColor.includes('#')) return parseInt ( htmlColor.replace("#","0x"), 16 )
